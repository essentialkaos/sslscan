...
func (s *SSLLabsSuite) TestInfo(c *check.C) {
	...
	c.Assert(api.Info.EngineVersion, check.Equals, "1.32.13")
...
func (s *SSLLabsSuite) TestAnalyze(c *check.C) {
	...
	c.Assert(details.Cert.Subject, check.Equals, "CN=ssllabs.com, OU=Production, O=\"Qualys, Inc.\", L=Foster City, ST=California, C=US")
	...
	c.Assert(details.Cert.IssuerSubject, check.Equals, "CN=DigiCert Global CA G2, O=DigiCert Inc, C=US")
	c.Assert(details.Cert.IssuerLabel, check.Equals, "DigiCert Global CA G2")
	c.Assert(details.Cert.SigAlg, check.Equals, "SHA256withRSA")
	c.Assert(details.Cert.RevocationInfo, check.Equals, 3)
	c.Assert(details.Cert.CRLURIs, check.DeepEquals, []string{"http://crl3.digicert.com/DigiCertGlobalCAG2.crl"})
	c.Assert(details.Cert.OCSPURIs, check.DeepEquals, []string{"http://ocsp.digicert.com"})
	...
	c.Assert(details.Cert.SCT, check.Equals, true)
	c.Assert(details.Cert.MustStaple, check.Equals, 0)
	c.Assert(details.Cert.SHA1Hash, check.Equals, "835fa74e3cecad06e82cb3469b7af8287edc59b2")
	c.Assert(details.Cert.PINSHA256, check.Equals, "Apy5nr74bAFaH6LW5jamLzig16emadx9yHSDqDIVGM4=")
	...
	c.Assert(details.Chain.Certs[0].Subject, check.Equals, "CN=ssllabs.com, OU=Production, O=\"Qualys, Inc.\", L=Foster City, ST=California, C=US")
	...
	c.Assert(details.Chain.Certs[0].IssuerSubject, check.Equals, "CN=DigiCert Global CA G2, O=DigiCert Inc, C=US")
	c.Assert(details.Chain.Certs[0].IssuerLabel, check.Equals, "DigiCert Global CA G2")
	...
	c.Assert(details.Chain.Certs[1].Subject, check.Equals, "CN=DigiCert Global CA G2, O=DigiCert Inc, C=US")
	c.Assert(details.Chain.Certs[1].Label, check.Equals, "DigiCert Global CA G2")
	...
	c.Assert(details.Chain.Certs[1].IssuerSubject, check.Equals, "CN=DigiCert Global Root G2, OU=www.digicert.com, O=DigiCert Inc, C=US")
	c.Assert(details.Chain.Certs[1].IssuerLabel, check.Equals, "DigiCert Global Root G2")
